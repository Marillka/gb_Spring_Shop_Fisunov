...
//    }
    ...
    public void createNewOrder(String username) {
            // создаем Order
            // Туда нужно запихнуть totalPrice, User, List<OrderItem>
            Order order = new Order();

            // достаем корзину из из сериса
            CartDto cart = cartServiceIntegration.getCurrentCart();

            // достаем юзера из сервиса
            User user = userService.findByUsername(username).get();

            // инжектим юзера и totalPrice в Order.
            order.setUser(user);
            order.setTotalPrice(cart.getTotalPrice());

            // создаем List<OrderItem>
            List<OrderItem> orderItems = new ArrayList<>();
            // пробегаемся по корзине и запихиваем все элементы в List<OrderItem>
